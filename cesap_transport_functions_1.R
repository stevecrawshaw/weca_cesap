...
get_la_dft_lookup <- function(){
  ...
  read_csv('https://storage.googleapis.com/dft-statistics/road-traffic/downloads/data-gov-uk/local_authority_traffic.csv', lazy = TRUE) %>% 
    filter(year == max(year)) %>% 
    select(Local_authority_id, name, ONS_code, year)
  
}

get_local_dft_data_tbl <- function(authorities_tbl, auth_grouping = c('weca', 'lep')){
...
rank_woe_ordinal <- function(tbl, year_col, meas_col){
  ...
  tbl %>% 
  filter({{year_col}} == max({{year_col}})) %>% 
  group_by(CAUTH22NM) %>% 
  summarise(mean_meas_col = mean({{meas_col}})) %>% 
  mutate(rank = rank(mean_meas_col)) %>% 
  subset(CAUTH22NM == "West of England" | CAUTH22NM == "West of England (inc NS)", rank) %>% 
  pull() %>% 
  toOrdinal()
  
}

plot_car_use_ca <- function(all_ca_traffic_data_tbl, year_col, meas_col){
...
all_ca_traffic_data_tbl %>% 
  ...
  summarise(mean_ratio = mean(cars_all_ratio),
            .groups = 'drop') %>% 
  ggplot(aes(x = year.x, y = mean_ratio, group = CAUTH22NM, colour = CAUTH22NM)
  ) +
  geom_line(linewidth = 2) + 
  scale_color_discrete() +
  gghighlight(CAUTH22NM == "West of England",
  ...
  labs(x = "Year",
       ...
       subtitle = glue("The West of England ranks {rank_woe_ordinal(all_ca_traffic_data_tbl, year_col = {{year_col}}, meas_col = {{meas_col}})} in car use. Lower is better"),
...
make_ca_emissions_list <-function(
  ...
  stopifnot(
    'incorrect code - should be weca or lep' = (auth_grouping %in% c('weca',
                                                                     'lep')),
    "sub sector(s) not in emissions table" = (sub_sectors %in% unique(uk_emissions_raw_tbl$la_ghg_sub_sector)))
  
      if(auth_grouping == "lep"){ # if north somerset to be included
      # add extra row to ca lookup table
    
    ns_row_tbl <- tibble(
      LAD22CD = "E06000024",
      LAD22NM = "North Somerset",
      CAUTH22CD = "E47000009",
      ...
      ca_dft_lookup <- bind_rows(ca_dft_lookup, ns_row_tbl) %>% 
        mutate(CAUTH22NM = if_else(CAUTH22NM == "West of England", "West of England (inc NS)", CAUTH22NM))
    
    }
    
      subset_tbl <- uk_emissions_raw_tbl %>%
      right_join(ca_dft_lookup,
                 by = join_by(local_authority_code == LAD22CD)) %>%
    ...
    co2_la_year_tbl <- subset_tbl %>%
      group_by(calendar_year, CAUTH22NM, local_authority_code) %>%
      summarise(kt_CO2e = sum(territorial_emissions_kt_co2e))
    
    per_cap_annual_sub_sector_emissions_tbl <- pop_la_year_tbl %>% 
      inner_join(co2_la_year_tbl,
                 by = join_by(local_authority_code == local_authority_code,
                              calendar_year == calendar_year)) %>% 
      group_by(CAUTH22NM, calendar_year) %>% 
      summarise(ca_pop = sum(la_pop),
...
rank_woe <- rank_woe_ordinal(ca_emissions_list$per_cap_annual_sub_sector_emissions_tbl, 
...
co2e_per_cap_woe_latest <- 
  ...
  ungroup() %>% 
  filter(str_detect(CAUTH22NM, "West of England"),
...
ca_emissions_list$per_cap_annual_sub_sector_emissions_tbl %>% 
  ...
  geom_line(aes(x = calendar_year,
                ...
                group = CAUTH22NM),
            linewidth = 2) +
  gghighlight(CAUTH22NM == "West of England" | CAUTH22NM == "West of England (inc NS)",
  ...
  labs(x = "Year",
       ...
       caption = glue("The {woe_name} is {rank_woe} \nlowest in CO<sub>2</sub> emissions at {round(co2e_per_cap_woe_latest, 2)} tonnes per person", .trim = FALSE),
       colour = "Combined\nAuthority") +
  theme(
